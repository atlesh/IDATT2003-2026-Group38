...
public class GameSaveMapper {
  ...
  private static GameSave.SavedTransaction mapTransaction(Transaction transaction) {
    ...
    GameSave.TransactionType type =
        transaction instanceof Purchase ? GameSave.TransactionType.PURCHASE :
  ...
  private static Transaction restoreTransaction(
    ...
    if (stock == null) {
      throw new InvalidSaveFileException(
          "Unknown stock symbol in saved transaction: " + stockSymbol);
    }
  ...
  private static void validateBigDecimals(List<BigDecimal> values, String context,
    ...
    for (int i = 0; i < values.size(); i++) {
      ...
      if (requirePositive && value.compareTo(BigDecimal.ZERO) <= 0) {
        throw new InvalidSaveFileException(
            context + " contains a non-positive value at index " + i);
      }
    }
  }
